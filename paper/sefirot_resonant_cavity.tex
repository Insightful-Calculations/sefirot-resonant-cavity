...
concentration is bimodal: every other mode carries at most $10\%$ at Tiferet,
...
\end{equation}
uniquely maximizes the Tiferet eigenvector concentration while preserving
$\bbZ_2$ parity structure. Three distinct coupling thresholds govern the
Daat--Tiferet interaction:
\begin{enumerate}[leftmargin=*,label=\textup{(\roman*)}]
\item $w^* \approx 1.035$: the \emph{optimal weight}, at which Tiferet
  concentration peaks at $88.6\%$---a net gain of $1.9$ percentage points
  above the no-Daat baseline of $86.7\%$.
\item $w \approx 1.358$: the \emph{baseline-crossing threshold}, above which
  the Daat-augmented graph yields \emph{lower} Tiferet concentration than
  the no-Daat graph. For $w > 1.358$, adding Daat coupling makes Tiferet
  worse, not better.
\item $w_c \approx 1.7752$: the \emph{50\%-crossover weight} (algebraic fixed
  point), at which Tiferet concentration falls to exactly $50\%$. $w_c$ is
  a root of a degree-$7$ polynomial with integer coefficients; numerical
  evidence suggests it is transcendental (no relation found to any known
  algebraic number of degree $\le 30$).
\end{enumerate}
The amplification window is therefore $w \in (0, 1.358)$; for $w > 1.358$
Daat already absorbs spectral mass from Tiferet. At $w = w_c \approx 1.7752$
the crossover reaches $50\%$; beyond this point Daat dominates the
...
fixed point), consistent with Design Rule~1.

\paragraph{Architectural interpretation.}
Daat functions as a \emph{feedback amplifier} only for weak coupling
$w < 1.358$: it receives cross-cutting analytics from both pillars (via the
Chokhmah--Binah and Chesed--Gevurah pairs) and feeds the synthesized
signal back to Tiferet, reinforcing the cavity. The optimal operating point
$w^* \approx 1.035$ balances amplification gain against the onset of
competition. The baseline-crossing threshold $w \approx 1.358$---not the
50\%-crossover $w_c \approx 1.7752$---is the practically relevant warning
boundary: beyond it, over-instrumenting analytics actively degrades
...
\item \emph{Optimal Daat Coupling} (Theorem~\ref{thm:daat}): The
  ...
  (to $88.6\%$ at optimal weight $w^* \approx 1.035$) while preserving
  $\bbZ_2$ structure. The amplification window closes at $w \approx 1.358$
  (baseline-crossing); the 50\%-crossover is $w_c \approx 1.7752$.
\end{enumerate}
